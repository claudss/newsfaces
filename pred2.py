...
def extract_features(img_path):
    ...
    if args.cnn:
        locs = face_locations(X_img,
                    model = "cnn",
                    number_of_times_to_upsample = args.n_scale)
    else:
        locs = face_locations(X_img, number_of_times_to_upsample = args.n_scale)
    if len(locs) == 0:
        return None, None
...
def draw_attributes(img_path, df):
    ...
    for row in df.iterrows():
        ...
        text_showed_race = "{}".format(race)
        text_showed_gender = "{}".format(gender)

        cv2.rectangle(img, (left, top), (right, bottom), (0, 0, 255), 2)
        font = cv2.FONT_HERSHEY_DUPLEX
        img_width = img.shape[1]
        cv2.putText(img, text_showed_race, (left+5, bottom-9), font, 0.5, (255, 255, 255), 1)
        cv2.putText(img, text_showed_gender, (left+5, bottom-20), font, 0.5, (255, 255, 255), 1)
    return img
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_scale', type = int,
                        default=1,
                        help = ("Set number of time to upsample."
                                "You will get more detected face"
                                "(default = 1"))
    parser.add_argument('--cnn', action = "store_true",
                        help = "Use CNN for face detection instead of HOG")
